...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (!body.name || typeof body.value !== 'number') {
      return withVersionHeader(apiErrorResponse(E_VALIDATION_FAILED, 'Invalid payload', 400, requestId));
    }

    if (!VALID_VITALS.has(body.name)) {
      return withVersionHeader(apiErrorResponse(E_VALIDATION_FAILED, 'Unknown vital', 400, requestId));
    }
    ...
    return withVersionHeader(NextResponse.json({ ok: true }, { headers: { 'x-request-id': requestId } }));
  } catch {
    return withVersionHeader(apiErrorResponse(E_INTERNAL, 'Failed to process vitals', 500, requestId));
  }
}
