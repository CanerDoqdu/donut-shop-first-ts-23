-- =============================================
-- 002: Extended Features — Stores, Loyalty, Gift Cards,
--      Subscriptions, Reviews, Referrals, Admin, Analytics
...
-- Store Inventory
DROP POLICY IF EXISTS "Store inventory viewable by everyone" ON store_inventory;
CREATE POLICY "Store inventory viewable by everyone"
...
RETURNS TEXT AS $$
BEGIN
  IF lifetime_pts >= 10000 THEN RETURN 'platinum';
  ELSIF lifetime_pts >= 5000 THEN RETURN 'gold';
  ELSIF lifetime_pts >= 2000 THEN RETURN 'silver';
  ELSE RETURN 'bronze';
  END IF;
END;
$$ LANGUAGE plpgsql;
...
BEGIN
  UPDATE products
  SET avg_rating   = (SELECT COALESCE(AVG(rating), 0) FROM reviews WHERE product_id = NEW.product_id AND is_approved = true),
...
DROP TRIGGER IF EXISTS award_points_on_order_paid ON orders;
